...
public class ChessGame {
  ...
  public void initializeBoard() {
    ...
    for (int i = 0; i < this.whitePlayer.getPieces().size(); i++) {
      board.getSquare(this.whitePlayer.getPieces().get(i).getX(),
          this.whitePlayer.getPieces().get(i).getY())
          .occupySquare(this.whitePlayer.getPieces().get(i));
    }
    for (int i = 0; i < this.blackPlayer.getPieces().size(); i++) {
      board.getSquare(this.blackPlayer.getPieces().get(i).getX(),
          this.blackPlayer.getPieces().get(i).getY())
